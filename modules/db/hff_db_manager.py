...
from .hff_db_mapper import UW, ART, ANC, POTTERY, SITE,  \
    MEDIA, \
    MEDIA_THUMB, MEDIATOENTITY, MEDIAVIEW, \
    PDF_ADMINISTRATOR
from .hff_db_update import DB_update
from .hff_utility import Utility
...
class Hff_db_management(object):
    ...
    def insert_uw_values(self, *arg):
        """Istanzia la classe US da hff_db_mapper"""
    ...
    def insert_anc_values(self, *arg):
        """Istanzia la classe ANC da hff_db_mapper"""
    ...
    def insert_pottery_values(self, *arg):
        """Istanzia la classe POTTERY da hff_db_mapper"""
    ...
    def insert_site_values(self, *arg):
        """Istanzia la classe SITE da hff_db_mapper"""
        sito = SITE(arg[0],
                    arg[1],
                    arg[2],
                    arg[3],
    ...
    def insert_media_values(self, *arg):
        """Istanzia la classe MEDIA da hff_db_mapper"""
        media = MEDIA(arg[0],
                      arg[1],
                      arg[2],
                      arg[3],
    ...
    def insert_media2entity_values(self, *arg):
        """Istanzia la classe MEDIATOENTITY da hff_db_mapper"""
    ...
    def insert_pdf_administrator_values(self, *arg):
        """Istanzia la classe PDF_ADMINISTRATOR da hff_db_mapper"""
    ...
    def execute_sql_create_db(self):
        ...
        rel_path = os.path.join(os.sep, 'query_sql', 'hff_create_db.sql')
    ...
    def execute_sql_create_spatialite_db(self):
        path = os.path.dirname(__file__)
        rel_path = os.path.join(os.sep, 'query_sql', 'hff_create_spatialite_db.sql')
        qyery_sql_path = '{}{}'.format(path, rel_path)
        create = open(qyery_sql_path, "r")
        stringa = create.read()
        create.close()
    ...
    def execute_sql_create_layers(self):
        path = os.path.dirname(__file__)
        rel_path = os.path.join(os.sep, 'query_sql', 'hff_layers_postgis.sql')
        qyery_sql_path = '{}{}'.format(path, rel_path)
        create = open(qyery_sql_path, "r")
        stringa = create.read()
        create.close()
    ...
    def select_quote_from_db_sql(self, sito, area, us):
        sql_query_string = ("SELECT * FROM hff_quote WHERE sito_q = '%s' AND area_q = '%s' AND us_q = '%s'") % (
        sito, area, us)
    ...
    def select_usneg_doc_from_db_sql(self, sito, tipo_doc, nome_doc):
        sql_query_string = (
                           "SELECT * FROM hff_us_negative_doc WHERE sito_n = '%s' AND  tipo_doc_n = '%s' AND nome_doc_n = '%s'") % (
                           sito, tipo_doc, nome_doc)
        res = self.engine.execute(sql_query_string)
        return res
...
# def main():
    # db = Hff_db_management('sqlite:////Users//Luca//HFF_DB_folder//hff_db.sqlite')
    # db.connection()
