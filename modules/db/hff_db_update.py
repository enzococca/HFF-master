...
from .hff_conn_strings import Connection
...
class DB_update(object):
    ...
    def update_table(self):
        ...
        # ####hff_thesaurus_sigle
        # table = Table("hff_thesaurus_sigle", self.metadata, autoload=True)
        # table_column_names_list = []
        # for i in table.columns:
            # table_column_names_list.append(str(i.name))

        # if not table_column_names_list.__contains__('lingua'):
            # self.engine.execute("ALTER TABLE hff_thesaurus_sigle ADD COLUMN lingua text DEFAULT '' ")
        ...
        # try:
            # self.engine.execute("ALTER TABLE hff_strutture_ipotesi ADD COLUMN nr_strut integer DEFAULT 0 ")
            # self.engine.execute(
                # "ALTER TABLE hff_strutture_ipotesi ADD COLUMN sigla_strut varchar(3) DEFAULT 'NoD'")
        # except:
            # pass
