...
from ..modules.gis.hff_pyqgis import Hff_pyqgis
from .US_USM import hff_US

MAIN_DIALOG_CLASS, _ = loadUiType(
    os.path.join(os.path.dirname(__file__), os.pardir, 'gui', 'ui', 'Gis_Time_controller.ui'))


class hff_Gis_Time_Controller(QDialog, MAIN_DIALOG_CLASS):
    ...
    def define_order_layer_value(self, v):
        try:
            ...
            QgsMessageLog.logMessage(
                "You must to load hff_us_view and/or select it from hff GeoDatabase" + str(e))
            self.iface.messageBar().pushMessage("Help",
                                                "You must to load hff_us_view and/or select it from hff GeoDatabase",
                                                level=Qgis.Warning)
